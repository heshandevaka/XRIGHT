...
from xright.models import Actor, ActorPhi3
...
class DeepspeedStrategy(ABC):
    ...
    def create_optimizer(self, model, **kwargs) -> Optimizer:
        if isinstance(model, Actor) or isinstance(model, ActorPhi3):
            model = model.model
        # Optimizer
    ...
    def _ds_init_train_model(self, model, optim, scheduler):
        is_actor = isinstance(model, Actor) or isinstance(model, ActorPhi3)
    ...
    def my_save_model(self, model: nn.Module, tokenizer, output_dir, **kwargs) -> None:
        if self.is_rank_0():
            os.makedirs(output_dir, exist_ok=True)
        ...
        if self.is_rank_0():
            ...
            if isinstance(model_to_save, PeftModel):
                # if 'Llama1' in output_dir:
                #     print("Llama model detected. Saving only the LoRA parameters as adapter.pth")
                #     lora_parameters = {name: param for name, param in model_to_save.named_parameters() if 'lora' in name}
                #     model_to_save.save_pretrained(output_dir, **kwargs)
                #     torch.save(
                #         lora_parameters,
                #         os.path.join(output_dir, "adapter.pth"),
                #     )
                # else:
                model_to_save.save_pretrained(output_dir, **kwargs)
                torch.save(
    ...
    def broadcast(self, data, src) -> int:
        is_cpu_tensor = data.device.type == "cpu"

        if is_cpu_tensor:
            data = data.to(torch.cuda.current_device())
        return dist.broadcast(data, src=src)
