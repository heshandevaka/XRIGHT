...
from xright.datasets import PromptDataset, SFTDataset
from xright.models import Actor, get_llm_for_sequence_regression
from xright.utils import blending_datasets, get_processor, get_strategy, get_tokenizer
...
def alpaca_eval_batch_generate(args):
    # configure strategy
    strategy = get_strategy(args)
    strategy.setup_distributed(timeout=timedelta(minutes=720))
    ...
    for prompts in pbar:
        ...
        for _ in range(N):
            ...
            for prompt, output in zip(prompts, outputs):
                output = output[len(prompt) :]
                output_dataset.append({"instruction": prompt, "output": output})
...
if __name__ == "__main__":
    ...
    if args.eval_task and args.eval_task == "generate_alpaca_eval":
        alpaca_eval_batch_generate(args)
