...
class SFT_DPO_MAXRIGHT_Trainer(ABC):
    """
    ...
    def fit(self, args):
        ...
        if args.max_eval_steps == -1:
            args.max_eval_steps = float("inf")  # do not do the ocasional simultaneous actual objective max evaluation
        ...
        # Initial setting up intial stale_loss values
        print("Setting intial stale loss values..")
        with torch.no_grad():
            ...
            stale_loss_0 = ((1-self.lambd) * (gpt_loss + aux_loss * self.args.aux_loss_coef - self.sft_opt)).item()
            ...
            stale_loss_1 = self.lambd *(preference_loss + aux_loss * self.args.aux_loss_coef - self.dpo_opt).item()
    
        stale_loss_0 = self.strategy.all_reduce(stale_loss_0)
        stale_loss_1 = self.strategy.all_reduce(stale_loss_1)   
        ...
        for epoch in range(self.epochs):
            ...
            for step in range(self.sft_train_dataloader.__len__()):
                ...
                # if simultaneous max evaluations for both objectives are done
                if args.max_eval_steps!=float("inf") and step%args.max_eval_steps==0:
                    with torch.no_grad():
                ...
                # sample objective based on stale loss values
                objective_index = np.argmax([stale_loss_0, stale_loss_1]) 
                ...
                if objective_index==0:
                    ...
                    # update stale loss value for updated obejctive
                    stale_loss_0 = (1-self.lambd) * (loss.item() - self.sft_opt)
                ...
                if objective_index==1:
                    ...
                    # update stale loss value for updated obejctive
                    stale_loss_1 = self.lambd * (loss.item() - self.dpo_opt)
