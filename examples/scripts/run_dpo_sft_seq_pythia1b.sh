...
for na in $NA_SET; do
   ...
   deepspeed --module xright.cli.train_dpo_seq \
      --save_path ./checkpoint/pythia-1b-beta=$beta-learning_rate=$learning_rate-seq-dpo-pref-ref-temp \
      --save_steps -1 \
      --logging_steps 1 \
      --eval_steps 100 \
   ...
   deepspeed --module xright.cli.train_sft_seq \
      ...
      --eval_steps 100 \
      --train_batch_size $train_batch_size \
      --pretrain ./checkpoint/pythia-1b-beta=$beta-learning_rate=$learning_rate-seq-dpo-pref-ref-temp \
      --ref_pretrain ./checkpoint/pythia-1b-beta=$beta-learning_rate=$learning_rate-epoch=$max_epochs-sft-pref-optim  \
      --bf16 \
      --max_epochs $((6-$na)) \
      --zero_stage 2 \
